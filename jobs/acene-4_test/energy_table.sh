...
pre_scr="scr/singlet/acene4_scr"
econst_scr=$(awk 'END{print}' "$pre_scr.FCIDUMP" | awk '{print $1}')
pre_std="std/singlet/acene4_std"
...
printf "\\hline\n" 
printf "\\multirow{5}{*}{\$ Acene-4 \$} & \$ {}^1A_g1 \$ & %3.6f & %3.6f & %3.6f & %3.6f\\\\\\ \n" "${qci_scr[0]}" "${dmrg_scr[0]}" "${qci_std[0]}" "${dmrg_std[0]}" 
...
pre_scr="scr/triplet/acene4_scr"
pre_std="std/triplet/acene4_std"
